...
Headquarter::Headquarter(State_Manager* game, sf::RenderWindow* _window) : State(game, _window)
{
	...
	Bouton.push_back(Button(CreateText("Recrute\nChevalier", font, 20), 
		CreateRectangle(1.f, {150,50}), sf::Color(150, 150, 150), { 100, 390 },
		[this] {
			if (Army.size() < 12) {
	...
	Bouton.push_back(Button(CreateText("Recrute\nAssasin", font, 20), 
		CreateRectangle(1.f, { 150,50 }), sf::Color(150, 150, 150), { 100, 490 },
		[this] {
			if (Army.size() < 12) {
				Army.push_back(Recruitment.Get_Box(Mercenary::Type::Assassin));
				auto* it = &Army.back();
	...
	Bouton.push_back(Button(CreateText("Recrute\nPretre", font, 20), 
		CreateRectangle(1.f, { 150,50 }), sf::Color(150, 150, 150), { 100, 590 },
		[this] {
			if (Army.size() < 12) {
	...
	Bouton.push_back(Button(CreateText("Recrute\nSorcier", font, 20), 
		CreateRectangle(1.f, { 150,50 }), sf::Color(150, 150, 150), { 100, 690 },
		[this] {
			if (Army.size() < 12) {
	...
	Bouton.push_back(Button(CreateText("Clear\nSquad", font, 20), 
		CreateRectangle(1.f, { 75,50 }), sf::Color(150, 150, 150), { 50, 50 },
		[this] {
			Joueur.Get_Squad().clear();
		}));

	Bouton.push_back(Button(CreateText("Dungeon", font, 60), CreateRectangle(1.f), { 960, 1040 },
		[this] {
			if (Joueur.Get_Squad().size() >= 2) {
				Game->PushState<MenuDungeon>(Game, window, &Joueur);
			}
		}));
}
...
void Headquarter::Resume()
{
	...
	for (Mercenary& Current : Army) {
		...
		Army_Button.push_back(Button(CreateText(tmp, font, 20),
			CreateRectangle(1.f, { 120,50 }), sf::Color::Blue, sf::Vector2f(1800, 60 * i), [this, it] {
				if (Joueur.Get_Squad().size() < 4) {
					bool find = false;
					for (auto& current : Joueur.Get_Squad())
						if (current == it)
